...
class Analyzer(restful.Resource):
    ...
    def post(self):
        ...
        status,generate_result=self.generate_code(contract_name,code)
        generate_result = json.dumps(generate_result)
    ...
    def generate_code(self,crypt_name,crypt_code):
        try:
            cryptfilepath=os.path.join("/home/xialb/dsl_project/Cryplang/Test",crypt_name+".crypt")
            cryptfilepath=str(cryptfilepath)
            solidityfilepath=os.path.join("/home/xialb/dsl_project/Cryplang/Test",crypt_name+".sol")
            solidityfilepath=str(solidityfilepath)
            generateresult={}#返回格式“code":.sol
            generateresult["code"]=[]
            
            with open(cryptfilepath,"w+") as fw:
                fw.write(crypt_code)
            fr=open(solidityfilepath,"w+")
            fr.close()
            
            subprocess.run("python /home/xialb/dsl_project/Cryplang/main.py %s %s"%(cryptfilepath,solidityfilepath),shell=True)
            mysol=open(solidityfilepath,"r")
            lines=mysol.readlines()
            Str=""
            for line in lines:
                Str+=line
